...
export default function AchievementsPage() {
  ...
  const enrichedAchievements = achievementMetadata.map((achievement) => {
    const earnedRecord = earnedAchievements.find((item) => item.name === achievement.name);
    return {
      ...achievement,
  ...
  const earnedCount = enrichedAchievements.filter((a) => a.earned).length;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="space-y-3">
        ...
        {loading && (
          <p className="text-sm text-foreground/50 font-medium">Loading your saved badges…</p>
        )}
      </div>
      ...
      {enrichedAchievements.some((a) => !a.earned) && (
        <div className="space-y-4">
